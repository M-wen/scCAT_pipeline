...
parser$add_argument('-b','--RNAbarcode', help='sample name')
...
barcodeTranslate=as.data.frame(fread(args$RNAbarcode,header=F))

raw_meta <- raw_meta[which(barcodeTranslate$V1%in%raw_meta$V1),]
...
table$Count=as.integer(table$BeadsCount/as.numeric(as.character(table$Num)))
...
p=ggplot(data = table, mapping = aes(x = factor(Num), y = Count, fill = Num)) +
  ...
  scale_fill_brewer(palette = 'Set2',labels = paste(table$Num," ",table$Count))+
  theme_bw()+
