...
def plot_jaccard_knee_frag():
    ...
    trace0 = Scattergl(
        ...
        line=dict(color="#1358A2",width=3)
    )
    trace1 = Scattergl(
        x = trace1_x,
        y = trace1_y,
    ...
    layout = go.Layout(xaxis=dict(type="log", gridcolor="lightgrey",title="Barcode in Rank-descending Order",color="black",showline=True,zeroline=True,linewidth=1,fixedrange= True,linecolor="black"),
                    yaxis = dict(type="log",title="Fragment counts per Barcode",gridcolor="lightgrey",linewidth=1,fixedrange= True,color="black",linecolor="black"),
    ...
    py.offline.plot(fig, filename = outpath+"/report/div/barcode_rank_atac.html",auto_open=False,config=config)
    fig2=py.offline.plot(fig, include_plotlyjs=False,show_link=False,output_type='div',config=config)
    fw = open(outpath+"/report/div/barcode_rank_atac.div",'w')
    fw.write(fig2)
    fw.close()
